...
def load_tp_config() -> TPConfig:
    ...
    Returns:
        TPConfig instance with configuration from environment variables
    """
    load_dotenv()
    return TPConfig()
...
def validate_tp_credentials(config: TPConfig) -> bool:
    """Check if TrainingPeaks credentials are properly configured.

    Args:
        config: TPConfig instance to validate
...
def update_tp_env_key(cookie: str) -> None:
    """Update the .env file with a new TrainingPeaks auth cookie.

    Args:
        cookie: New TP_AUTH_COOKIE value to save
    """
